<?php
include('koneksi.php');
$sql = mysqli_query($koneksi,"SELECT * FROM bidan");
while($row = mysqli_fetch_array($sql)){
	$query = mysqli_query($koneksi,"SELECT hari FROM bidan");
...
<br>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				...
				datasets: [{
					data: [
					<?php 
					$jumlah_senin = mysqli_query($koneksi,"select * from bidan where hari = 'Monday'");
					echo mysqli_num_rows($jumlah_senin);
					?>,
					<?php 
					$jumlah_selasa = mysqli_query($koneksi,"select * from bidan where hari = 'Tuesday'");
					echo mysqli_num_rows($jumlah_selasa);
					?>,
					<?php 
					$jumlah_rabu = mysqli_query($koneksi,"select * from bidan where hari = 'Wednesday'");
					echo mysqli_num_rows($jumlah_rabu);
					?>,
					<?php 
					$jumlah_kamis = mysqli_query($koneksi,"select * from bidan where hari = 'Thursday'");
					echo mysqli_num_rows($jumlah_kamis);
					?>,
					<?php 
					$jumlah_jumat = mysqli_query($koneksi,"select * from bidan where hari = 'Friday'");
					echo mysqli_num_rows($jumlah_jumat);
					?>,
					<?php 
					$jumlah_sabtu = mysqli_query($koneksi,"select * from bidan where hari= 'Saturday'");
					echo mysqli_num_rows($jumlah_sabtu);
					?>,
					<?php 
					$jumlah_minggu = mysqli_query($koneksi,"select * from bidan where hari = 'Sunday'");
	...
	<center>
		<a href="bidan.php" style="color: white">Tampilkan tabel</a>
	</center>
