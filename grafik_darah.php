<?php
include('koneksi.php');
$sql = mysqli_query($koneksi,"SELECT * FROM tes_darah");
while($row = mysqli_fetch_array($sql)){
	$query = mysqli_query($koneksi,"SELECT hari FROM tes_darah");
...
<html>
<head>
	<title>Grafik Kunjungan Tes Darah</title>
...
<br>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				...
				datasets: [{
					data: [
					<?php 
					$jumlah_senin = mysqli_query($koneksi,"select * from tes_darah where hari = 'Monday'");
					echo mysqli_num_rows($jumlah_senin);
					?>,
					<?php 
					$jumlah_selasa = mysqli_query($koneksi,"select * from tes_darah where hari = 'Tuesday'");
					echo mysqli_num_rows($jumlah_selasa);
					?>,
					<?php 
					$jumlah_rabu = mysqli_query($koneksi,"select * from tes_darah where hari = 'Wednesday'");
					echo mysqli_num_rows($jumlah_rabu);
					?>,
					<?php 
					$jumlah_kamis = mysqli_query($koneksi,"select * from tes_darah where hari = 'Thursday'");
					echo mysqli_num_rows($jumlah_kamis);
					?>,
					<?php 
					$jumlah_jumat = mysqli_query($koneksi,"select * from tes_darah where hari = 'Friday'");
					echo mysqli_num_rows($jumlah_jumat);
					?>,
					<?php 
					$jumlah_sabtu = mysqli_query($koneksi,"select * from tes_darah where hari= 'Saturday'");
					echo mysqli_num_rows($jumlah_sabtu);
					?>,
					<?php 
					$jumlah_minggu = mysqli_query($koneksi,"select * from tes_darah where hari = 'Sunday'");
	...
	<center>
		<a href="darah.php" style="color: white">Tampilkan tabel</a>
	</center>
